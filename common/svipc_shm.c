...
static int getfree_slot(slot_master* m) {
   ...
   for (i=0;i<m->numslots;i++) {
      if (m->sse[i].slot_shmid==0) return i;
   }
   return -1;
}
...
int svipc_shm_info(long key, long details) {
   ...
   for (i=0;i<m->numslots;i++) {
      ...
      if (details && m->sse[i].slot_shmid!=0) {
         ...
         if (typeid==SVIPC_CHAR) fprintf (stderr, "   char ");
         else if (typeid==SVIPC_SHORT) fprintf (stderr, "   short ");
         else if (typeid==SVIPC_INT) fprintf (stderr, "   int ");
         else if (typeid==SVIPC_LONG) fprintf (stderr, "   long ");
         else if (typeid==SVIPC_FLOAT) fprintf (stderr, "   float ");
         else if (typeid==SVIPC_DOUBLE) fprintf (stderr, "   double ");
...
int svipc_shm_write(long key, char *id, slot_array *a, int publish) {
   ...
   long totalnumber = 1;
   for (i=0; i<countdims; i++) totalnumber *= *(a->number+i);
   ...
   // wake up
   if ( publish ) status |= publish_snapshot(&sss);
